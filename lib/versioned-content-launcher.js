...
function readOptions(opts) {
  ...
  options = {
    ...
    root  : opts.root || "" // alternative prefix for generated links
  };
}
  ...
  Promise.all([ getAllVersions(context), getCurrentVersion(context) ])
    .then(function(data) {
      ...
      for (var i = 0; i < versions.length; i++) {
        response += "<li><a href='" + options.root + req.baseUrl + "?version=" + versions[i] + "'>" + versions[i] + "</a>" + (versions[i] == current ? " (current)" : "") + "</li>";
      }
